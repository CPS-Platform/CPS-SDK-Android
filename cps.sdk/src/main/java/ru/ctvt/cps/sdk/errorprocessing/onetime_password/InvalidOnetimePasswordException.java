 ...
 * Когда одноразовый пароль не проходит проверку
 * Created by Nokolya on 25.04.2017.
 */

public class InvalidOnetimePasswordException extends OnetimePasswordException {

    private void init(){
        error_code_postfix = EXCEPTION_POSTFIX_INVALID_ONETIME_PASSWORD;
    }

    public InvalidOnetimePasswordException(){
        super(EXCEPTION_STRING_INVALID_ONETIME_PASSWORD);
        init();
    }

    public InvalidOnetimePasswordException(String message){
        super(Strings.isNullOrEmpty(message)? EXCEPTION_STRING_INVALID_ONETIME_PASSWORD: message);
        init();
    }

    public InvalidOnetimePasswordException(String message, Throwable cause){
        super(Strings.isNullOrEmpty(message)? EXCEPTION_STRING_INVALID_ONETIME_PASSWORD: message, cause);
        init();
    }

    public InvalidOnetimePasswordException(Throwable cause){
        super(EXCEPTION_STRING_INVALID_ONETIME_PASSWORD, cause);
        init();
    }
    ...
    public static int getExpectedErrorCode(){
        return EXCEPTION_PREFIX_ONETIME_PASSWORD+EXCEPTION_POSTFIX_INVALID_ONETIME_PASSWORD;
    }
