 ...
 * 3100
 * img_not_found
 * Когда изображения не найдено
 * Created by Nokolya on 26.04.2017.
 */

public class ImageNotFoundException extends ImageProcessingException {

    private void init(){
        error_code_postfix = EXCEPTION_POSTFIX_IMG_NOT_FOUND;
    }

    public ImageNotFoundException(){
        super(EXCEPTION_STRING_IMG_NOT_FOUND);
        init();
    }

    public ImageNotFoundException(String message){
        super(Strings.isNullOrEmpty(message)? EXCEPTION_STRING_IMG_NOT_FOUND: message);
        init();
    }

    public ImageNotFoundException(String message, Throwable cause){
        super(Strings.isNullOrEmpty(message)? EXCEPTION_STRING_IMG_NOT_FOUND: message, cause);
        init();
    }

    public ImageNotFoundException(Throwable cause){
        super(EXCEPTION_STRING_IMG_NOT_FOUND, cause);
        init();
    }
    ...
    public static int getExpectedErrorCode(){
        return EXCEPTION_PREFIX_IMAGE_PROCESSING+EXCEPTION_POSTFIX_IMG_NOT_FOUND;
    }
